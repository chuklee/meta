...
class Base:
    def __init__(self, taux_base, neurone_username, neurone_password, new = True, pc_dividende = 2, neurones = [], reserve = 0, length = 1, identifiant = "None"):
        #Parametre de base
        #BASICS
        ...
        self.reserve = reserve #Reserve d'argent de la base pour les futurs investissement
        self.length = length #Nombre de neurones présent dans la base
        self.identifiant = identifiant # L'identifiant de la base pour la retrouver dans la database, attention de base il est à None, c'est peut être un truc à patcher
        #NEURONES
        self.neurones = neurones #Représente les différents neurones de la base
        if(new):
